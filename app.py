...
- Optimizada para cargar rápidamente en Streamlit Cloud
"""
...
if 'datos_cacheados' not in st.session_state:
    st.session_state.datos_cacheados = None
...
# ============================================================================
# FUNCIÓN AUXILIAR: OBTENER DATOS CON CACHÉ
# ============================================================================
def obtener_datos_con_cache():
    """Obtiene datos una sola vez y los cachea en session_state"""
    try:
        datos = gestor.obtener_todas(solo_interes=True)
        st.session_state.datos_cacheados = datos
...
# Obtener datos (se ejecuta una sola vez por sesión a menos que se rescrapeee)
if st.session_state.datos_cacheados is None:
    todas = obtener_datos_con_cache()
else:
    todas = st.session_state.datos_cacheados
...
with st.sidebar:
    ...
    st.metric(label="Total en CSV", value=len(todas))
    
    abiertas = [c for c in todas if c.estado == EstadoConvocatoria.ABIERTA]
